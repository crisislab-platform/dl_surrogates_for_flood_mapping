...
BATCH_SIZES=(64)
LEARNING_RATES=(0.01)
PHYSICS_WEIGHTS=(0.001 0.01 0.1 0.5 1)
EPOCHS=50
PATIENCE=10
TUNING_MODE=true
...
for LEARNING_RATE in "${LEARNING_RATES[@]}"; do
  ...
  for PHYSICS_WEIGHT  in "${PHYSICS_WEIGHTS[@]}"; do
    echo "Configuration: Learning Rate=${LEARNING_RATE}, Physics Weight=${PHYSICS_WEIGHT}"
    echo "------------------------------------------------------------------"
    ...
    for BATCH_SIZE in "${BATCH_SIZES[@]}"; do
      ...
      echo "Processing group 4 folds (${GROUP4[*]}) in parallel"
      parallel --line-buffer CUDA_VISIBLE_DEVICES=${DEVICE} python3.11 main.py train \
        --model PICNN1D_V1 \
        --lag ${LAG} \
        --horizon ${HORIZON} \
