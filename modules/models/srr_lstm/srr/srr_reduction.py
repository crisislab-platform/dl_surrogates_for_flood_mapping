...
class SDRReducer():
    def __init__(self):
        ...
        # RL selection
        self.resample_rate_mcl = 0.1
        self.resample_rate_sdr_thalwegs = 0.1
        self.ouput_rls_file_name = "representative_locations.shp"
    ...
    def sdr_searching(self):
        ...
        if self.require_rep_thalweg_selection:
            starttime = time.time()
            output_file_name = f"{self.result_dir}/{self.sdr_file_thalwegs[:-4]}_before_rep_thal_selec.shp"
            ...
            logger.info(f"SDR search completed in {time.time() - starttime:.2f} seconds. Results saved to {self.sdr_file}")
        else:
            starttime = time.time()
    ...
    def sdr_searching_mcl(self):
        starttime = time.time()
        starting_points = np.array(self.starting_coords).reshape(1,2)
        Traj_search(self.result_dir, starting_points, self.dem_asc_file, self.stopping_values_dem, self.customised_searching_win_size)\
    ...
    def sdr_rl(self):
        starttime = time.time()
        mcl_file_path = os.path.join(self.result_dir, self.sdr_file_mcl)
        thalweg_file_path = os.path.join(self.result_dir, self.sdr_file_thalwegs)
        extract_rep_pts(mcl_file_path, thalweg_file_path, self.resample_rate_mcl, self.resample_rate_sdr_thalwegs, self.result_dir, self.ouput_rls_file_name)
