...
from modules.model_trainer.ussr_1dcnn.spatial_reduction_module.gdal_lib import *
from modules.model_trainer.ussr_1dcnn.spatial_reduction_module.nc_func import *
import os
...
def save_pts_to_shp(pts, outfile, bounds_file=None):
    # Create a GeoDataFrame from the points
    if bounds_file:
        with rasterio.open(bounds_file) as src:
            bounds = src.bounds
    else:
        bounds = None
        
    #use the bounds to create a geodataframe
    geometry = [Point(coord[0], coord[1]) for coord in pts]
...
def save_trajs_to_shp_lines(trajs, outfile):
    # Create geometries for each trajectory
    geometries = []
    for key in trajs.keys():
        line_points = [key] + trajs[key]
