...
import os
import numpy as np
from modules.lib.constants import DATA_DIR as DATA_DIR, OUTPUT_DIR, SIMULATION_DATA_DIR, RUN_DIR, PLOTS_OUTPUT_DIR, DEM_FILE
import logging
import rasterio
...
logger = logging.getLogger("Exploration Plots")
...
def create_flood_animation():
    ...
    dem_file = os.path.join(DATA_DIR, "Carlisle_5m.asc")
...
def plot_extent_prediction(run_id=None, idx="0145"):
    ...
    # Create and save the visualization
    output_file = os.path.join(PLOTS_OUTPUT_DIR, "maximum_flood_extent_cnn1d.png")
...
def plot_extents_on_same_image():
    ...
    output_file = os.path.join(PLOTS_OUTPUT_DIR, f"model_comparison_{idx}.png")
...
def visualise_area_check_map():
    file = os.path.join(PLOTS_OUTPUT_DIR, "area_check.tif")
    output_file = os.path.join(PLOTS_OUTPUT_DIR, "area_check_visualization.png")
    dem_file = os.path.join(SIMULATION_DATA_DIR, "Carlisle_5m.asc")
...
def plot_study_area_clean(output_filename=None):
    ...
    output_file = os.path.join(PLOTS_OUTPUT_DIR, output_filename)
    dem_file = os.path.join(SIMULATION_DATA_DIR, "Carlisle_5m.asc")
...
def plot_study_area():
    ...
    study_area = os.getenv("STUDY_AREA", "Carlisle")
    output_filename = f"{study_area}_dem.png"
    ...
    try:
        with rasterio.open(DEM_FILE) as src:
            ...
            # max, min and mean elevation
            dem_nodata = src.nodata
            dem_valid = dem_data[dem_data != dem_nodata]
            dem_max = np.max(dem_valid)
            dem_min = np.min(dem_valid)
            dem_mean = np.mean(dem_valid)
            logger.info(f"DEM Elevation - Max: {dem_max} m, Min: {dem_min} m, Mean: {dem_mean:.2f} m")
            
            #visualise the distribution of elevation values
            plt.figure(figsize=(8, 5))
            plt.hist(dem_valid.flatten(), bins=50, color='skyblue', edgecolor='black')
            plt.title(f'{study_area} DEM Elevation Distribution', fontsize=16, fontweight='bold')
            plt.xlabel('Elevation (m)', fontsize=14)
            plt.ylabel('Frequency', fontsize=14)
            plt.grid(True, alpha=0.3, linestyle='--')
            hist_output_file = os.path.join(PLOTS_OUTPUT_DIR, f"{study_area}_dem_histogram.png")
            plt.savefig(hist_output_file, dpi=300, bbox_inches='tight')
            logger.info(f"DEM elevation histogram saved to {hist_output_file}")
            plt.close()
            ...
            ax.set_title(f'{study_area} Study Area DEM', fontsize=16, fontweight='bold')
            ax.grid(True, alpha=0.3, linestyle='--')
            ax.set_xticklabels([])
            ax.set_yticklabels([])  
            plt.tight_layout()
            plt.savefig(output_file, dpi=300, bbox_inches='tight')
            logger.info(f"Study area DEM saved to {output_file}")
            plt.close()
            return output_file
    except Exception as e:
        logger.error(f"Error loading DEM file: {e}")
